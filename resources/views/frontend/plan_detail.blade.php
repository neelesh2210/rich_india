                        ...
                        <div class="inner">
                            ...
                            @if (!Auth::guard('web')->user())
                                <div class="tg-button-wrap">
                                    <a href="{{ route('checkout') }}?slug={{ $plan_detail->slug }}"
                                        class="btn arrow-btn">Buy Now <i class="fas fa-arrow-right"></i></a>
                                </div>
                            @endif
                        </div>
        ...
        <div class="container">
            <div class="row">
                @foreach ($plan_detail->course_ids as $course_id)
                    @php
                        $course = App\Models\Admin\Course::where('delete_status', '0')
                            ->where('status', '1')
                            ->where('id', $course_id)
                            ->first();
                    @endphp
                    <div class="col-md-3">
                        <div class="courses__item shine__animate-item">
                            ...
                            <div class="courses__item-content">
                                ...
                                <h5 class="title"><a
                                        href="{{ route('course.detail', $course->slug) }}">{{ $course->name }}</a></h5>
                            </div>
                        </div>
                    </div>
                @endforeach
